...
- **バス乗降時刻表示**: バス停での乗車時刻と降車時刻を明確に表示（`boardingTime`と`alightingTime`）
- **バス乗り換え対応**: 直通バスがない場合、主要ハブ経由の2区間ルートを自動生成（5分乗り換え時間）

### 重要なアルゴリズムの詳細

#### バス乗降時刻の計算 (`optimizerService.js`)
- **乗車時刻 (boardingTime)**: GTFSの`departure_time`から取得
- **降車時刻 (alightingTime)**: 乗車時刻 + バス乗車時間（分）
- **時刻形式**: "HH:MM:SS" (例: "09:15:00")
- **計算ロジック**: `parseTime()`で分に変換 → 移動時間を加算 → `formatTime()`で"HH:MM:SS"に戻す

#### バス停検索とルート決定 (`gtfsService.js`)
